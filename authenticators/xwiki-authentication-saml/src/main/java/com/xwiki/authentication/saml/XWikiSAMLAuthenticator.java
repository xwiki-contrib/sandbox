...
public class XWikiSAMLAuthenticator extends XWikiAuthServiceImpl
{
	...
	public boolean checkSAMLResponse(XWikiContext context) throws XWikiException
	{
		...
		String sql = "select distinct doc.fullName from XWikiDocument as doc, BaseObject as obj, StringProperty as nameidprop where doc.fullName=obj.name and obj.className='XWiki.SAMLAuthClass' and obj.id=nameidprop.id.id and nameidprop.id.name='nameid' and nameidprop.value='" + nameID + "'";
	...
	public XWikiUser checkAuth(XWikiContext context) throws XWikiException
	{
		...
		if (samlUserName==null) {
			...
			if (context.getRequest().getCookie("username")!=null || context.getAction().equals("logout") || context.getAction().startsWith("login")) {
				if (LOG.isDebugEnabled()) {
	...
	private boolean getXWikiUsernameRuleCapitalization(XWikiContext context) {
		String capitalize =
			context.getWiki().Param("xwiki.authentication.saml.xwiki_user_rule_capitalize", DEFAULT_XWIKI_USERNAME_RULE_CAPITALIZE);
