...
/**
 * XWiki Watch feed entry source retrieval and rendering function.
 * 
 * @version $Id$
 */
@Component("FEEDENTRY")
public class FeedEntryService implements IOTargetService
{
    private final String FEED_CLASS_NAME = "XWiki.FeedEntryClass";
    ...
    public Source getSource(CharSequence documentName, XWikiContext deprecatedContext) throws IOServiceException
    {
        try {
            ...
            BaseObject feed = document.getObject(FEED_CLASS_NAME);
            String s = feed.getField("content").toString().replace("\r", "");
            s = StringEscapeUtils.unescapeXml(s);
            s = StringEscapeUtils.unescapeXml(s);
            return new SourceImpl(s);
     ...
     */
    public CharSequence getRenderedContent(CharSequence documentName, Source source, XWikiContext deprecatedContext)
