...
XWiki.widgets.Suggest = Class.create({
  ...
  doAjaxRequests: function ()
  {
    ...
    // create ajax request
    var sources = [ this.sources ].flatten().compact();

    if (sources.length > 1 && typeof this.resultContainer == 'undefined') {
    ...
    for (var i=0;i<sources.length;i++) {
      ...
      if (sources.length > 1 && typeof this.resultContainer.down('.results' + source.id) == 'undefined') {
        var sourceContainer = new Element('div', {'class' : 'results results' + source.id});
        var sourceHeader = new Element('div', {'class':'sourceName'});
        if (typeof source.icon != 'undefined') {
          sourceHeader.insert( new Element('img', {'src': source.icon}) );
        }
      ...
      else if(typeof this.resultContainer != 'undefined' && typeof this.resultContainer.down('.results' + source.id) != 'undefined') {
      ...
      var url = source.script + source.varname + "=" + encodeURI(this.fld.value.strip());
  ...
  createContainer: function()
  {
    ...
    var relativeSize = this.options.relativeSize || 1;
    var width = this.fld.offsetWidth * relativeSize;
    var align = this.options.align || "left";

    if (align == 'right') {
      var left = pos.left - ((relativeSize - 1) * this.fld.offsetWidth);
  ...
  createList: function(arr, source)
  {
    ...
    if (this.sources.length == 1 && typeof this.resultContainer == 'undefined') {
      this.createContainer();
    }

    if (this.sources.length > 1) {
      var div = this.resultContainer.down(".results" + source.id).down('.sourceContent');
  ...
  resetTimeout: function()
  {
    clearTimeout(this.toID);
    var pointer = this;
    this.toID = setTimeout(function () { pointer.clearSuggestions() }, this.options.resetTimeout || 5000);
  },
