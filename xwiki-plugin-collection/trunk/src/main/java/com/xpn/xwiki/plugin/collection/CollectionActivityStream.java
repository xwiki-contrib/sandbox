                    ...
                    for (Iterator<String> iterator2 = collections.iterator(); iterator2.hasNext();) {
                        ...
                        if (pageDoc != null && !pageDoc.isNew()) {
                            ...
                            try {
                                ...
                                LOG.error("Failed to store event: page " + page + " has been added to page " +
                                    newdoc.getFullName() + " in collection " + collName);
                            }

                            if (LOG.isDebugEnabled()) {
                                LOG.debug("[COLL] Page " + page + " has been added to page " + newdoc.getFullName() +
                    ...
                    for (Iterator<String> iterator2 = collections.iterator(); iterator2.hasNext();) {
                        ...
                        if (pageDoc != null && !pageDoc.isNew()) {
                            ...
                            try {
                                ...
                                LOG.error("Failed to store event: page " + page + " has been removed to page " +
                                    newdoc.getFullName() + " in collection " + collName);
                            }

                            if (LOG.isDebugEnabled()) {
                                LOG.debug(
                                    "[COLL] Page " + page + " has been removed from page " + newdoc.getFullName() +
                                        " in collection " + collName);
                            }
                        }
                    }
            ...
            for (Iterator<String> iterator2 = collections.iterator(); iterator2.hasNext();) {
                ...
                if (event == XWikiDocChangeNotificationInterface.EVENT_CHANGE) {
                    if (olddoc == null || olddoc.isNew()) {
                        try {
                            ...
                            LOG.debug("Failed to store event: page " + newdoc.getFullName() +
                                " has been created in collection " + collName);
                        }

                        if (LOG.isDebugEnabled()) {
                            LOG.debug(
                                "[COLL] Page " + newdoc.getFullName() + " has been created in collection " + collName);
                        }
                        ...
                        try {
                            ...
                            LOG.debug("Failed to store event: page " + newdoc.getFullName() +
                                " has been deleted in collection " + collName);
                        }

                        if (LOG.isDebugEnabled()) {
                            LOG.debug(
                                "[COLL] Page " + newdoc.getFullName() + " has been deleted in collection " + collName);
                        }
                    } else {
                        try {
                            ...
                            LOG.debug("Failed to store event: page " + newdoc.getFullName() +
                                " has been modified in collection " + collName);
                        }

                        if (LOG.isDebugEnabled()) {
                            LOG.debug(
                                "[COLL] Page " + newdoc.getFullName() + " has been modified in collection " + collName);
                        }
                    }
                }
                ...
                if (event == XWikiDocChangeNotificationInterface.EVENT_NEW) {
                    try {
                        ...
                        LOG.debug(
                            "Failed to store event: page " + newdoc.getFullName() + " has been created in collection " +
                                collName);
                    }
                ...
                if (event == XWikiDocChangeNotificationInterface.EVENT_DELETE) {
                    try {
                        ...
                        LOG.debug(
                            "Failed to store event: page " + newdoc.getFullName() + " has been deleted in collection " +
                                collName);
                    }
